...
def test_unpack_string_inputs(dims, coords, expected):
    factors = "L"
    factor_prices = "w"
    factors, factor_prices = unpack_string_inputs(
        factors, factor_prices, dims=dims, coords=coords
    )
...
def test_computation_of_CES():
    ...
    def CES_Y(L, K, A, alpha, epsilon):
        return A * (
            (alpha) * L ** ((epsilon - 1) / epsilon)
            + (1 - alpha) * K ** ((epsilon - 1) / epsilon)
        ) ** (epsilon / (epsilon - 1))
    ...
    def CES_demand(factor_price, output, output_price, TFP, factor_share, epsilon):
        return (
            output
            / TFP
            * ((factor_share) * output_price * TFP / factor_price) ** epsilon
        )
...
def test_dixit_stiglitz(A, alpha, backend):
    ...
    if backend == "numba":
        ...
        prod_expected_inner = f"({alpha_str}X ** ((epsilon - 1) / epsilon)).sum() ** (epsilon / (epsilon - 1))"
    else:
        assert False
...
def test_dixit_stiglitz_computation(backend):
    def dx_Y(X, A, alpha, epsilon):
        return A * (alpha * X ** ((epsilon - 1) / epsilon)).sum() ** (
            epsilon / (epsilon - 1)
        )
    ...
    if backend == "numba":
        ...
        exact = dx_X(
            Y=1, A=1, alpha=np.full(k, 1 / k), P_Y=1, P_X=np.ones(k), epsilon=3
        )
        np.testing.assert_allclose(sympy_out, exact)
        ...
        Y, inputs = compile_Y()
        f_Y = pytensor.function(
            inputs, Y, on_unused_input="ignore", mode="FAST_COMPILE"
        )
        ...
        X, inputs = compile_X()
        f_X = pytensor.function(
            inputs, X, on_unused_input="ignore", mode="FAST_COMPILE"
        )
        expected = dx_X(
            Y=1, A=1, alpha=np.full(7, 1 / 7), P_Y=1, P_X=np.ones(7), epsilon=3
        )
        np.testing.assert_allclose(
            f_X(1, 1, np.ones(7), 1, np.full(7, 1 / 7), 3), expected
        )
...
def test_2d_leontief(backend: BACKEND_TYPE, coords: dict):
    ...
    if backend == "numba":
        ...
        sympy_eq = sp.parse_expr(
            eq.equation, local_dict=local_dict, transformations="all"
        )
        ...
        eqs = expand_obj_by_indices(
            eq, coords=coords, dims=None, on_unused_dim="ignore"
        )
        ...
        expected_outputs = [
            f"Y[{dim_2}] - ("
            + " + ".join(
                [f"P_X[{dim_1}] * X[{dim_1}, {dim_2}]" for dim_1 in coords["i"]]
            )
            + f")/P_Y[{dim_2}]"
        ...
        assert (
            factor_demands[0] == f"{factors[0]} = {factor_shares[0]} * {output}[None]"
        )
    else:
        assert False
...
def test_leonteif_1d_computation(backend):
    ...
    if backend == "numba":
        ...
        for factor_demand, f_expected in zip(
            factor_demands, [leontief_VA, leontief_VC]
        ):
            eq = sp.parse_expr(
                factor_demand, transformations="all", local_dict=local_dict
            )
            f_eq = sp.lambdify(inputs, eq.rhs)
            sympy_out = f_eq(
                VA=np.arange(k),
        ...
        f_VA = pytensor.function(
            inputs, VA, on_unused_input="ignore", mode="FAST_COMPILE"
        )
        ...
        f_VC = pytensor.function(
            inputs, VC, on_unused_input="ignore", mode="FAST_COMPILE"
        )
...
def test_2d_leontief_computation(backend, dims, coords):
    ...
    if backend == "numba":
        ...
        sympy_out = f_eq(
            X=X_val, P_X=X_prices, Y=Y, P_Y=P_Y, phi_X=phi_X_vals, j=np.arange(j_len)
        )
        exact = leontief_Y(X=X_val, P_X=X_prices, P_Y=P_Y)
        np.testing.assert_allclose(sympy_out, exact)

        eq = sp.parse_expr(
            factor_demands[0], transformations="all", local_dict=local_dict
        )
