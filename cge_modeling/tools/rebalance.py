...
def _crossentropy_method_setup(
    df, use_grad=True, use_hess=False, use_hessp=True, use_constraint_grad=True, mode=None
) -> tuple[
    Callable,
    Callable,
    Callable,
    ...
    cross_entropy = (x * (pt.log(x) - pt.log(nonzero_values))).sum()
...
def balance_SAM(
    ...
    **minimize_kwargs,
    ...
    res = optimize.minimize(
        ...
        method=method,
        tol=tol,
        options=dict(maxiter=25_000),
    )

    if not res.success:
        _log.info(f"Optimization failed. Return values may not represent a balanced SAM")
