...
from cge_modeling.sympy_tools import sub_all_eqs, symbol
...
def latex_print_equations(
    equation_info, variables, variable_info, parameters, param_info, return_latex=False
):
    var_subs = {
        var: sp.Symbol(d.get("latex_name"), **var._assumptions)
        for d, var in zip(variable_info, variables)
    }
    param_subs = {
        param: sp.Symbol(d.get("latex_name"), **param._assumptions)
        for d, param in zip(param_info, parameters)
    }

    sub_dict = var_subs | param_subs
    names, equations = [d.get("name") for d in equation_info], [
        d.get("equation") for d in equation_info
    ]
